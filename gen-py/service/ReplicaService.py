...
class Iface(object):
    ...
    def listFiles(self, query_others):
        """
        Parameters:
         - query_others
...
class Client(Iface):
    ...
    def listFiles(self, query_others):
        ...
        """
        self.send_listFiles(query_others)
        return self.recv_listFiles()

    def send_listFiles(self, query_others):
        self._oprot.writeMessageBegin('listFiles', TMessageType.CALL, self._seqid)
        args = listFiles_args()
        args.query_others = query_others
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()
...
class Processor(Iface, TProcessor):
    ...
    def process_listFiles(self, seqid, iprot, oprot):
        ...
        result = listFiles_result()
        try:
            result.success = self._handler.listFiles(args.query_others)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
...
class listFiles_args(object):
    ...
    def __init__(self, query_others=None,):
        self.query_others = query_others
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 1:
                if ftype == TType.BOOL:
                    self.query_others = iprot.readBool()
                else:
                    iprot.skip(ftype)
            else:
                iprot.skip(ftype)
    ...
    def write(self, oprot):
        ...
        if self.query_others is not None:
            oprot.writeFieldBegin('query_others', TType.BOOL, 1)
            oprot.writeBool(self.query_others)
            oprot.writeFieldEnd()
        oprot.writeFieldStop()
        oprot.writeStructEnd()
...
listFiles_args.thrift_spec = (
    None,  # 0
    (1, TType.BOOL, 'query_others', None, None, ),  # 1
)
